...
#include "Joystick_X.h"
    ...
    #define Joystick_X_SetP4PinDriveMode(shift, mode)  \
    do { \
        Joystick_X_PC =   (Joystick_X_PC & \
                                (uint32)(~(uint32)(Joystick_X_DRIVE_MODE_IND_MASK << \
                                (Joystick_X_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Joystick_X_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Joystick_X_SetP4PinDriveMode(shift, mode)  \
        do { \
            Joystick_X_USBIO_CTRL_REG = (Joystick_X_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Joystick_X_DRIVE_MODE_IND_MASK << \
                                    (Joystick_X_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Joystick_X_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Joystick_X_SetP4PinDriveMode(Joystick_X__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Joystick_X_DR & (uint8)(~Joystick_X_MASK));
    drVal = (drVal | ((uint8)(value << Joystick_X_SHIFT) & Joystick_X_MASK));
    Joystick_X_DR = (uint32)drVal;
}
...
{
    return (uint8)((Joystick_X_PS & Joystick_X_MASK) >> Joystick_X_SHIFT);
}
...
{
    return (uint8)((Joystick_X_DR & Joystick_X_MASK) >> Joystick_X_SHIFT);
}
...
void Joystick_X_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Joystick_X_INTCFG & (uint32)(~(uint32)position);
    Joystick_X_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Joystick_X_INTSTAT & Joystick_X_MASK);
	Joystick_X_INTSTAT = maskedStatus;
    return maskedStatus >> Joystick_X_SHIFT;
}
